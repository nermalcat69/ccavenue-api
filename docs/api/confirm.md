...
Confirm API call allows you to confirm a pending order. Only confirmed orders are settled into the merchant's account. An order older than 12 days is automatically cancelled. Once an order has been auto-cancelled by the system, it cannot be confirmed. You can optionally confirm only part of an order. Remember, you can do so only once. Remaining funds will be refunded to the credit card or debit card or net banking account that was originally charged.
